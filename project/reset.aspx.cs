    ...
    public partial class reset : System.Web.UI.Page
    {
        ...
        protected void submitBtn_Click(object sender, EventArgs e)
        {
            if (con.State == ConnectionState.Closed)
                con.Open();
            string matchQry = "select user from [users] where email='" + email.Text.ToString() + "' and is_active=1";
            ...
            else
            {
                ...
                con.Open();
                string updateQry = "update [users] set pass_otp='" + activation_code.ToString() + "'";
        ...
        {
            con.Open();
            string matchQry = "select user from [users] where email='" + email.Text.ToString() + "' and pass_otp='" + newOTP.Text.ToString() + "'";
            SqlCommand matchCmd = new SqlCommand(matchQry, con);
            SqlDataReader matchReader = matchCmd.ExecuteReader();
            if (matchReader.Read())
            {
                ...
                con.Open();
                string updateQry = "update [users] set pass_otp='0'";
                SqlCommand updateCmd = new SqlCommand(updateQry, con);
                updateCmd.ExecuteNonQuery();
                con.Close();
        ...
        {
            con.Open();
            string updateQry = "update [users] set password='" + MyEncrypt(pass.Text.ToString()) + "' where email='" + email.Text.ToString() + "'";
            SqlCommand updateCmd = new SqlCommand(updateQry, con);
            updateCmd.ExecuteNonQuery();
            con.Close();
